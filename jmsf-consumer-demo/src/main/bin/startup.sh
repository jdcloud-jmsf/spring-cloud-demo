...
JPDA=${JPDA:-""}
SGM_OPTS=${SGM_OPTS:-"$SGM_OPTS"}
JAVA_OPT="-server -XX:+UseG1GC -Xmx${XMX} -Xss${XSS} -XX:MaxGCPauseMillis=${MPM} -Dfile.encoding=UTF-8 -XX:+ParallelRefProcEnabled"
JAVA_OPT="${JAVA_OPT} -XX:+DisableExplicitGC -XX:+PrintGC -XX:+PrintGCDetails -Xloggc:${LOG_DIR}/gc.log" # -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=10 -XX:GCLogFileSize=20M -XX:+PrintGCDateStamps -XX:+PrintGCTimeStamps -XX:+PrintGCApplicationStoppedTime
JAVA_OPT="${JAVA_OPT} -XX:-OmitStackTraceInFastThrow -XX:+HeapDumpOnOutOfMemoryError -Djava.awt.headless=true ${SGM_OPTS} ${JPDA}"
...
# 打印启动命令
#STARTUP_LOG="${STARTUP_LOG}Application startup command: nohup java ${JAVA_OPT} -jar -Dloader.path=config ${BASE_PATH}/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} > ${LOG_PATH} 2>&1 &\n"
STARTUP_LOG="${STARTUP_LOG}Application startup command: nohup java ${JAVA_OPT} -jar -Dloader.path=config ${BASE_PATH}/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} &\n"

#======================================================================
# 执行启动命令：后台启动项目,并将日志输出到项目根目录下的logs文件夹下
#======================================================================
#nohup java ${JAVA_OPT} -jar -Dloader.path=config ${BASE_PATH}/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} > ${LOG_PATH} 2>&1 &
nohup java ${JAVA_OPT} -jar -Dloader.path=config ${BASE_PATH}/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} 2>&1 &
